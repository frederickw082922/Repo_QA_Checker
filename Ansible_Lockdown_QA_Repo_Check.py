...
class ReportGenerator:
    ...
    def _html(self) -> str:
        ...
        for r in self.results:
            ...
            desc = CHECK_DESCRIPTIONS.get(r.name, "")
            desc_html = (
                f"<div style='font-size:0.85rem;color:#6c757d;"
        ...
        for r in self.results:
            ...
            overview_rows.append(
                ...
                f"<td style='font-size:0.85rem;color:#6c757d'>{_html_escape(d)}</td>"
...
<style>
  ...
  .check-section {{ background: #fff; border: 1px solid #dee2e6; border-radius: 8px;
                    box-shadow: 0 2px 4px rgba(0,0,0,.1); margin-bottom: 20px;
                    overflow: hidden; }}
  ...
  .toggle-arrow {{ transition: transform 0.2s; font-size: 0.8rem; color: #6c757d; }}
