...
- **`check_shell_pipefail.py`:** Read-only checker (stdlib only) for `ansible.builtin.shell` layout: `set -o pipefail` as first block line plus `args: executable: "{{ <prefix>_shell_executable }}"`. Reuses `fix_shell_pipefail.scan_file` so rules match the fix script. Auto-detects `*_shell_executable` from role vars. Supports `--all` / `--compact` batch mode. Already wired in `run_all_checks.sh`.
