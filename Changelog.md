...
Meta Validate moves in step: across the same fleet **13 repos declare the Tyto Athene parent in `meta/main.yml`** and now warn, against 3 already on Quantum Sky. Four further repos warn for reasons that predate this release and are unchanged by it - two co-branded SUSE15 roles and two naming no parent at all. Meta Validate returns `WARN`, which only affects the exit code under `--strict`; without it the run still exits 0.
